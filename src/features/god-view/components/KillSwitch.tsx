    ...
    >
        ...
        <div className="flex items-center gap-3">
          ...
          <div>
            ...
            <p className="text-[#6e7d93] text-xs">
              Multi-Sig required: CEO + Co-signer. Halts all platform operations.
            </p>
          </div>
        </div>
        ...
        {status === "active" && (
          <div className="flex items-center gap-3">
            <div className="flex items-center gap-2 px-3 py-1.5 rounded-full bg-[rgba(156,42,42,0.2)] border border-[rgba(156,42,42,0.5)]">
              ...
              <span className="text-[#9C2A2A] text-xs font-medium">KILL SWITCH ACTIVE</span>
            </div>
            <button
              onClick={deactivate}
      ...
      {status === "confirming" && (
        <div className="mt-4 space-y-3 border-t border-[rgba(156,42,42,0.2)] pt-4">
          <div>
            <label className="block text-xs text-[#A8B5C8] mb-1">Reason (required)</label>
            <textarea
              value={reason}
              onChange={(e) => setReason(e.target.value)}
              rows={2}
              placeholder="State reason for kill switch activation..."
          ...
          <div>
            <label className="block text-xs text-[#A8B5C8] mb-1">
              Co-signer Email (Multi-Sig)
            </label>
            <input
              type="email"
              value={cosignerEmail}
              onChange={(e) => setCosignerEmail(e.target.value)}
              placeholder="cosigner@iface.global"
          ...
          <div className="flex gap-2">
            ...
            >
              {(status as string) === "loading" ? "Activating..." : "Confirm Activation"}
            </button>
            <button
